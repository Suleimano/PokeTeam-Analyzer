...
def stream_team_analysis(team, api_key):
    ...
    payload = json.dumps({
        "model": "openai/gpt-oss-120b",                  # <-- the good free model
        "messages": [{"role": "user", "content": prompt}],
        "stream": True,
    }).encode("utf-8")

    req = urllib.request.Request(
        "https://api.groq.com/openai/v1/chat/completions",   # <-- back to Groq
        data=payload,
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {api_key}",             # <-- your Groq key
            "User-Agent": "PokeTeamAnalyzer/0.1",
        },
    )
    ...
    with urllib.request.urlopen(req) as r:
        for line in r:
            ...
            if not line.startswith("data: "):
                continue
            data = line[6:]
            if data == "[DONE]":                          # <-- OpenAI sends this to end the stream
                break
            chunk = json.loads(data)
            try:
                text = chunk["choices"][0]["delta"]["content"]
